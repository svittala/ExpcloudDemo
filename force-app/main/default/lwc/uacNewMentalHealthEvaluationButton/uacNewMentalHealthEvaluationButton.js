import LBL_RECORD_TYPES from '@salesforce/label/c.UAC_newMentalHealthEvaluationButton';
...
export default class UacNewMentalHealthEvaluationButton extends NavigationMixin
(LightningElement) {

  @api
  recordId;
  tableTitle = 'New Mental Health Evaluation';
  selectedRecordType;
  @track
  imeObjectInfo;
  @track
  ...
  wiredIMEObjectInfo({data, error}) {
    if (data) {
      ...
      Object.keys(data.recordTypeInfos).forEach((key) => {
        if (LBL_RECORD_TYPES.includes(data.recordTypeInfos[key].name)) {
    ...
    if (this.selectedRecordType === undefined) {
      ...
      createRecord(recordInput)
          .then(healthEvaluation => {
            ...
            this.dispatchEvent(new ShowToastEvent({
              title: 'Success',
              message: 'Mental Health Evaluation Created',
              variant: 'success'
            }));
