...
import getFileList from '@salesforce/apex/UAC_fileListController.getFileList';
...
export default class UacFileList extends NavigationMixin(LightningElement) {
  @api recordId;
  @api height;
  @api recordTypeName;
  @api componentLabel;
    ...
    if (actionName === 'view') {
      if (!this.useInCommunity) {
        ...
        if (baseUrl.indexOf('/s/') != -1) {
          let downloadURL = baseUrl.split('/s/')[0];
          downloadURL += '/sfc/servlet.shepherd/document/download/' + row.ContentDocumentId
              + '?operationContext=S1';
          this[NavigationMixin.Navigate](
              {type: 'standard__webPage', attributes: {url: downloadURL}}, false);
        }
      }
    }
  }
